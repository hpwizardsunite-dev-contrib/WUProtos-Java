...
1. Pull latest WUProtos and test build
  * `git submodule update --init --recursive && cd WUProtos && git pull origin master && cd .. `
2. Build project 
  * `mvn install`
  * ---OR---
  ...
  * --- OR---
  * `git submodule update --init --recursive && cd WUProtos && git pull origin master && cd .. && git commit -a -m  'update protos' && docker run -it --rm --name my-maven-project  -v "C:\Users\celan\.m2":/root/.m2 -v "F:\PotterBattler\WUProtos-Java":/usr/src/mymaven -w /usr/src/mymaven maven:3.2-jdk-8 mvn clean install`
